...
export const register = (userData) => async (dispatch) => {
  ...
  try {
    ...
    if (user && user.jwt) {
      ...
      console.warn("⚠️ JWT missing in signup response:", user);
    }
...
export const login = (userData) => async (dispatch) => {
  ...
  try {
    ...
    if (user && user.jwt) {
      ...
      console.warn("⚠️ JWT missing in login response:", user);
    }
...
export const getUser = (token) => async (dispatch) => {
  ...
  if (!token || !token.startsWith("ey")) {
    dispatch({ type: GET_USER_FAILURE, payload: "Invalid token format" });
    return;
  }
